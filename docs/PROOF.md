...
separate machines is demonstrated separately by the multi-device browser flow (the `/net`
page over a blind relay): a real DKG and a FROST signature run across independent
devices and verify, but that path does not yet broadcast. Neither limit is hidden; both
are the honest gap between "the ceremony is real" and "the ceremony was run distributed
and settled on-chain in one motion".
