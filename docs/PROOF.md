...
| **Opaque from the outside, with a live payment inside** (#476): a 2-of-3 vault proposed, approved, signed and sent while an observer holding only the vault id polled the helper throughout. Every private read answered `401` and `/api/vault` returned a byte-identical 253-byte response before, during and after - a vault mid-payment was indistinguishable from an idle one. The chain shows only a **V6** (Ironwood) transaction with nothing transparent; the observation is attested by the probe, not the block. **This row cited "the probe log" for three weeks while no such log was in the repository** - it rested on the operator's word while appearing to rest on evidence. The gate itself is now checkable by anyone, against their own vault, with `scripts/probe-opacity.mjs` (`docs/proof/2026-09-21-id-buys-metadata.md`). What that script cannot repeat is the *mid-payment* half recorded here, which still rests on the run described above. | `3fa08dceaa1d0a8f553ee68f6ddcf879b90f3cc7a19c370ace023ef5b2ecf81e` | 3,473,869 |
| **Every governance write authenticated** (#288): a 2-of-3 vault where the proposal, the approval and the send were each signed by an Ed25519 key derived from the seat's FROST share, and refused without one. The chain shows an ordinary shielded V6 transaction and cannot show this: what attests it is the same helper answering `401 this vault requires a signed proposal` and `401 ... signed send` to the identical request made without a signature, verified on staging and again in production the same day. | `47e4e5dddedf08f6e92e25a69c17f7198a652e25e458107ffce2313a347ef291` | 3,475,607 |
| **A 2-of-3 vault signed with one seat deliberately absent, after the seat-poisoning fix landed** (#399/#515): made on the deployed web app at 12:19 UTC on 2026-09-15, three hours after the fix merged. That is the shape the fix is about: a vault that signs with fewer members than it has, where a seat nobody vouched for could previously be chosen for the threshold set and produce an aggregate no share had made. **What attests each half is different, and the difference is the point.** The chain shows a shielded V6 transaction and can show neither. The 2-of-3 shape IS publicly checkable, from the coordinator's open vault endpoint, which reports the threshold and the member count without any credential. That a seat was left absent is attested by the operator who ran it and by nothing else; an earlier version of this row asserted it with no attestation at all, which is why the distinction is now written down. | `7d6b3dec5a39ebca1c8228304d5c347ab36b7f4283910b7acccd333aed7fc346` | 3,484,231 |
| **The device checked that the payment was the approved one before signing it** (#281): a 2-of-2 vault sending 0.0001 ZEC, built so the transaction carries CHANGE, which is the case that matters. Before this a device proved only that it signed the sighash of the PCZT it held (H1), never that the PCZT paid what the quorum approved, so a coordinator assembling a valid transaction to its own address produced a request every check passed and the only thing standing there was a human reading a preview. The chain cannot show the check: what attests it is that the change output goes to the vault's own internal receiver, which differs from its receive address, and the device recognised it as its own rather than refusing a legitimate payment. | `075ecfe9f8664634ee906ac1cd1949edf60f77b81dc51afebf1203ddd662f685` | 3,484,695 |
