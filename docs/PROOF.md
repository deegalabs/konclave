...
| **Opaque from the outside, with a live payment inside** (#476): a 2-of-3 vault proposed, approved, signed and sent while an observer holding only the vault id polled the helper throughout. Every private read answered `401` and `/api/vault` returned a byte-identical 253-byte response before, during and after - a vault mid-payment was indistinguishable from an idle one. The chain shows only a **V6** (Ironwood) transaction with nothing transparent; the observation is attested by the probe log, not the block. | `3fa08dceaa1d0a8f553ee68f6ddcf879b90f3cc7a19c370ace023ef5b2ecf81e` | 3,473,869 |
