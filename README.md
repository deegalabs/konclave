...
- ✅ **On mainnet, seven independently verifiable txids** (`node scripts/verify-proof.mjs`, or the
  ...
  vault**; and a **send from a vault whose key was generated by real DKG** (three-participant DKG
  ceremony, key **never reconstituted**), then funded and spent by a FROST ceremony; and, on
  **NU6.3 / Ironwood** activation day, an **Orchard→Ironwood migration** plus the **first spend from
  the Ironwood pool** (both **V6/NU6.3**, 2-of-3 FROST). Honest note: the
  quorum payment, payroll, and fresh-vault txids used a **trusted-dealer** vault; only the dedicated
  **DKG-vault send** came from a key born by real **DKG**.
