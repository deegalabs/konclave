...
- `orchestrator/src/*.rs`: the destructive suite (186 tests): money, the proposal state
  machine, validation, store, the loopback bridge (`handle`/`handle_secured`), sealing,
  payroll, DKG, send, the blind relay, and the Architecture B signing-request protocol.
...
engine binaries are built. See the destructive scenarios that must pass in
