...
/** Locale-aware numeric date from a real unix timestamp (seconds): DD/MM/YYYY in pt-BR,
 *  MM/DD/YYYY in en (each locale's convention). '-' when absent/invalid, never NaN.
 *  Year-qualified: a ledger handed to an accountant must distinguish 2026 from 2027.
 *  Defaults to pt-BR so existing callers/tests are unchanged. */
export function fmtDate(unix?: number, locale: 'pt-BR' | 'en' = 'pt-BR'): string {
  if (!isRealUnix(unix)) return '-'
