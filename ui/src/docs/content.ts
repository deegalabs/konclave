  ...
  },
  {
    ...
    blocks: [
      ...
      { k: 'h', t: { 'pt-BR': 'Provado vs pendente', en: 'Proven vs pending' } },
      {
        ...
        items: [
          {
            'pt-BR': '**Na mainnet, 7 txids verificáveis** (`node scripts/verify-proof.mjs` ou a tela /proof): um pagamento por quórum 2-de-3 (proposto/aprovado no app, assinado por FROST, shares lacrados em repouso); uma folha privada (uma tx Orchard blindada com 3 saídas, cada uma com memo criptografado, 2-de-3 FROST); um pagamento reproduzido ponta a ponta de um cofre criado e financiado do zero; um **envio a partir de um cofre gerado por DKG real** (cerimônia DKG de 3 participantes, chave nunca reconstituída), financiado e gasto por FROST; e, no dia da ativação do NU6.3/Ironwood, uma **migração Orchard→Ironwood** mais o **primeiro gasto DO pool Ironwood** (ambas V6/NU6.3, 2-de-3 FROST). Nota honesta: o pagamento por quórum, a folha e o cofre-novo usaram trusted-dealer; só o envio do cofre DKG veio de uma chave nascida por DKG real.',
            en: '**On mainnet, 7 verifiable txids** (`node scripts/verify-proof.mjs` or the /proof page): a 2-of-3 quorum payment (proposed/approved in the app, FROST-signed, shares sealed at rest); a private payroll (one shielded Orchard tx with 3 outputs, each with an encrypted memo, 2-of-3 FROST); a payment reproduced end to end from a freshly created and funded vault; a **send from a real DKG-generated vault** (three-participant DKG ceremony, key never reconstituted), funded and spent by FROST; and, on NU6.3/Ironwood activation day, an **Orchard→Ironwood migration** plus the **first spend FROM the Ironwood pool** (both V6/NU6.3, 2-of-3 FROST). Honest note: the quorum payment, payroll, and fresh vault used trusted-dealer; only the DKG-vault send came from a key born by real DKG.',
          },
          {
